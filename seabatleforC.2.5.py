...
class Field:
    ...
    def add_ship(self,
                 ship):  # Размещение кораблей (учитывая, что точка не занята, а если это так, то срабатывает
        # исключение)
        ...
        for d in ship.fields:
            for dx, dy in near:
                ...
                if not (self.out(cur)) and cur not in self.busy:
                    if verb:
                        self.field[cur.x][
                            cur.y] =color.PURPLE+"*"+color.END  # Символ ввиде точки, показывающий пользователю, что точка занята
    ...
    def shot(self, d):
        ...
        for ship in self.ships:
            if d in ship.fields:
                ship.lives -= 1
                self.field[d.x][d.y] = color.PURPLE+"X"+color.END  # Корабль поражен
                if ship.lives == 0:
                    self.count += 1
                    ...
                    print("Корабль ранен!")
                    return True
        self.field[d.x][d.y] = color.PURPLE+"*"+color.END
        print("Мазила!")
        return False
...
class Game:
    player_1 = input(color.YELLOW+'Давайте знакомиться! Меня зовут Адмирал. Как Ваше имя? \n'+color.END)
    ...
    #  Приветствие
    def greet(self):
        print(color.YELLOW+f'Приятно познокомиться, {Game.player_1}! Предлагаю сыграть со мной в игру "МОРСКОЙ БОЙ".'+color.END)
        print(color.YELLOW+"Выигрывает тот, кто поразит первым все корабли противника."+color.END)
        print(
            color.YELLOW+"Формат ввода координат числовой: x - отвечает за горизонталь, y - отвечает за вертикаль. Координаты вводятся через пробел."+color.END)
